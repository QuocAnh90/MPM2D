function Particle=Mohr_Coulomb_rotation(SolidModel,Particle,Time)
...
%% Rotation
Defgrad_Tensor = [Particle.defgrad(p,1) Particle.defgrad(p,2); Particle.defgrad(p,3) Particle.defgrad(p,4)];

% House holder
% R=Defgrad_Tensor; %Start with R=A
% Q=eye(2); %Set Q as the identity matrix
% x=zeros(2,1);
% x(1:2,1)=R(1:2,1);
% g=norm(x);
% v=x; v(1)=x(1)+g;
...
% Generate R with positive diagonals
R = qr(Defgrad_Tensor);
D = diag(sign(diag(R)));
R = D*R;
...
Particle.stress(2,p) = Stress_Tensor(2,2);
Particle.stress(3,p) = Stress_Tensor(1,2);
...
% Rotation back stress tensor Sigma = R Sigma R^T
Stress_Tensor = R * [Particle.stress(1,p) Particle.stress(3,p); Particle.stress(3,p) Particle.stress(2,p)] * R.'; 
% Reloading stress
Particle.stress(1,p) = Stress_Tensor(1,1);
Particle.stress(2,p) = Stress_Tensor(2,2);
